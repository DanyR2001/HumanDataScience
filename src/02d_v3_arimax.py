#!/usr/bin/env python3
"""
02d_v3_arimax.py  ─  Metodo 3: ITS / ARIMAX (come il paper)
=============================================================
Implementazione Python dell'approccio Interrupted Time Series (ITS) con
modello ARIMA/ARIMAX, generalizzando il metodo di Masena & Shongwe (2024)
...
Output:
  data/plots/its/{mode}/v3_arimax/              (se mode=fixed)
  data/plots/its/detected/{detect}/v3_arimax/   (se mode=detected)
    plot_{evento}_{carburante}.png
    v3_arimax_results.csv
"""
...
def _make_approach(label: str, pre: pd.Series, post: pd.Series,
                   order: tuple, consumption: float, aic: float, bic: float) -> dict:
    """Helper comune: fit pre-only, forecast post, calcola metriche."""
    ...
    return {
        ...
        "gain_meur": gain,
        "ci_gain_lo": float((post.values - ci_hi).sum() * consumption / 1e6),
        "ci_gain_hi": float((post.values - ci_lo).sum() * consumption / 1e6),
...
def compute_gain_from_counterfactual(
    actual_post: np.ndarray, counterfactual: np.ndarray, daily_consumption: float
) -> float:
    return float((actual_post - counterfactual).sum() * daily_consumption / 1e6)


def approach_a_pure_arima(pre, post, order, consumption):
    """A: Auto-ARIMA AIC — selezione automatica (p,d,q) su pre.

    Modello ARIMA classico: cattura l'autocorrelazione a bassa frequenza della
    serie pre-periodo.  Il forecast converge alla media incondizionata della pre
    (serie stazionarie) o segue il drift stimato (d>0).
    """
    if not HAS_SM:
    ...
    return _make_approach("A: Auto-ARIMA AIC (pre-only forecast)",
                          pre, post, order_a, consumption, aic, bic)


def approach_b_holt_winters(pre: pd.Series, post: pd.Series,
                             consumption: float) -> dict:
    """B: Holt-Winters ETS con trend additivo — estrapolazione del trend pre.

    Ipotesi: la serie mostra un andamento tendenziale nel pre-periodo
    (crescita/discesa), che sarebbe continuato in assenza dello shock.
    ExponentialSmoothing(trend='add') stima livello + slope al termine del pre
    e li proietta in avanti, producendo un controfattuale inclinato — visivamente
    molto diverso da A (che converge alla media).

    La smoothing iniziale usa ottimizzazione dei parametri α, β sul pre.
    Il CI approssimato è basato sulla dev. standard dei residui in-sample.
    """
    n   = len(post)
    ...
    if HAS_SM:
        try:
            ...
            # CI basato su incertezza cumulativa della previsione h passi avanti
            ...
            aic     = float(hw.aic)
            bic     = float(hw.bic)
            pre_resid = np.asarray(hw.resid)
        except Exception:
            # Fallback: media mobile lineare
    ...
    return {
        ...
        "_pre_resid": pre_resid,
        **m,
    }


def approach_c_ols_trend(pre: pd.Series, post: pd.Series,
                          consumption: float) -> dict:
    """C: OLS trend lineare — regressione y=a+b·t sulla pre, estesa al post.

    Ipotesi: il margine segue un trend lineare deterministico stabile che, in
    assenza dello shock, sarebbe continuato.  Il modello è il più semplice
    possibile (2 parametri) e massimalmente trasparente: è la retta dei minimi
    quadrati del pre-periodo, estrapolata n passi in avanti.

    Metodologicamente equivalente alla componente di trend dei modelli V1/V2,
    ma inserita qui per comparazione diretta con A (ARIMA, mean-reverting)
    e B (Holt-Winters, trend smussato/adattivo).
    """
    n = len(post)
    z = stats.norm.ppf(1 - CI_ALPHA / 2)

    # Regressione OLS: t = 0,1,...,T_pre-1
    ...
    # Proiezione: t = T_pre, T_pre+1, ...
    t_post = np.arange(len(pre), len(pre) + n, dtype=float)
    fc     = a + b * t_post

    # Residui in-sample per il CI
    ...
    # AIC/BIC OLS approssimato (2 parametri)
...
def _plot_its(
    ...
    ax_ser.set_title(
        f"[V3-ARIMAX / mode={mode}]  {fuel_key.capitalize()} – {ev_name}\n"
    ...
    for key, res in results.items():
        ...
        cum = pd.Series(
            np.cumsum(res["extra"]) * DAILY_CONSUMPTION_L[fuel_key] / 1e6,
            index=post.index
        )
...
def main() -> None:
    ...
    if mode == "detected":
        OUT_DIR = _OUT_BASE / "detected" / detect_target / "v3_arimax"
    else:
        OUT_DIR = _OUT_BASE / mode / "v3_arimax"
    OUT_DIR.mkdir(parents=True, exist_ok=True)

    print("═"*70)
    print(f"  02d_v3_arimax.py  –  Metodo 3: ITS/ARIMAX  [mode={mode}]")
    if mode == "fixed":
    ...
    for ev_name, ev in EVENTS.items():
        ...
        fig.suptitle(
            f"[Metodo 3 – ITS/ARIMAX / mode={mode}]  {ev_name}\n{ev['label']}",
        ...
        for row_idx, (fuel_key, (col_name, fuel_color)) in enumerate(FUELS.items()):
            ...
            res_a = approach_a_pure_arima(pre, post, order, consumption)
            res_b = approach_b_holt_winters(pre, post, consumption)
            res_c = approach_c_ols_trend(pre, post, consumption)
            ...
            # Per ARIMA residui si usa LB (non BG che richiede matrice OLS)
            ...
            plot_residual_diagnostics(
                resid=pre_resid,
                dates=pre.index,
                title=(f"[V3-ARIMAX] Diagnostica residui ARIMA{order} pre-periodo\n"
            ...
            if sarima is not None:
                ...
                sarima_gain  = float(sarima_extra.sum() * consumption / 1e6)
                ...
                plot_sarima_diagnostics(
                    resid=sarima["resid"],
                    dates=pre.index,
                    title=(f"[V3-ARIMAX / SARIMA{sarima['order']}]  "
                ...
                sarima_res_s = {
                    ...
                    "ci_gain_lo": float((post.values - sarima["ci_hi"]).sum() * consumption / 1e6),
                    "ci_gain_hi": float((post.values - sarima["ci_lo"]).sum() * consumption / 1e6),
    ...
    if rows:
        ...
        csv_out = OUT_DIR / "v3_arimax_results.csv"
