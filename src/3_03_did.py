...
3. SE Newey-West (HAC) invece di HC3
   Con dati settimanali autocorrelati, HC3 sottostima le SE.
   Usiamo Newey-West con lag automatico: L = ⌈4(T/100)^(2/9)⌉.
...
def _wholesale_series(fuel: str) -> pd.Series:
    ...
    Eurobob ARA (benzina) e London Gas Oil ICE (diesel) sono prezzi
    paneuropei: lo stesso mercato wholesale si applica a IT, DE, SE, ecc.
...
def _did_ols(it_mg: pd.Series, ct_mg: pd.Series,
    ...
    try:
        ols = sm.OLS(y, X).fit(cov_type="HAC", cov_kwds={"maxlags": lags, "use_correction": True})
    except Exception:
        return None
...
if not df_did.empty:
    ...
    fig.suptitle(
        ...
        "SE Newey-West (HAC)  |  Panel bilanciato (inner join per data)\n"
...
if not df_did.empty and IT_MARGINS:
    ...
    fig.suptitle(
        "Sanity check: serie margini IT (linea) vs controlli EU (banda)\n"
        f"Wholesale: {'futures Eurobob/GasOil' if HAS_EUROBOB else 'proxy Brent/yield'}",
        fontsize=10, fontweight="bold"
    )
    plt.tight_layout()
