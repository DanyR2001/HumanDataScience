...
Note sul modello MCMC:
  - τ ~ Beta(2,2) su [x_min, x_max]: prior leggermente lontano dai bordi.
  - ν ~ Gamma(2, 0.1): E[ν]=20, evita ν→1 (Cauchy) e ν→∞ (Normale).
...
def _bayesian_nuts(x: np.ndarray, y: np.ndarray, cfg: dict) -> dict:
    """
    Modello piecewise-lineare Bayesiano con likelihood Student-T.
    Campionato con NUTS via PyMC.
    τ ~ Beta(2,2) per evitare soluzioni ai bordi.
    ν ~ Gamma(2, 0.1) per code adattive (non Cauchy, non Normale).
    """
    n      = len(x)
    sd_y   = float(np.std(y))
