...
FAMILY_LABELS = {
    "A": "H₀(i)   — livello reale vs 2019",
    "B": "H₀(ii)  — salto pre→post shock",
    "C": "H₀(iii) — specificità italiana (DiD)",
}
        ...
        for ev in sub_up["evento"].dropna().unique():
            for paese in sub_up["paese_controllo"].dropna().unique():
                ...
                print(f"     {flag} {ev[:22]} vs {paese:<10}: "
                      f"{rej_ep}/{len(ep)} rigettati  PTA✓={n_pta_ok}/{len(ep)}")
...
for fam, res in family_summary.items():
    ...
    print(f"  {res['label']:<44}: {status}  ({res['n_rej']}/{res['n']})")

any_rej = any(r["rejected"] for r in family_summary.values() if r["n"] > 0)
print()
if any_rej:
    rej_fams = [f for f, r in family_summary.items() if r.get("rejected")]
    ...
    print("    Nessuna evidenza di extraprofitti anomali sui carburanti italiani.")
...
if os.path.exists(neff_path):
  ...
  ⚠  ROBUSTEZZA n_eff:
     ...
     HAC corregge la SE per autocorrelazione, ma non aumenta la potenza.
""")
...
for ev in events:
    for fuel in fuels:
        ...
        if not sub_b.empty:
            r = sub_b.iloc[0]
            row["B_delta_nom"]   = round(float(r.get("delta", np.nan)), 4)
